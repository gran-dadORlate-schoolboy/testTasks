...
public class CircleFactory  implements  FigureFactory{
    ...
    public Figure createFigure() {
        ...
        int x = (int) (300 +(Math.random() * 800));
        int y = (int) (300 +(Math.random() * 800));
        int wid = (int) (1 +(Math.random() * 300));
        int hight = (int) (1 +(Math.random() * 300));
        if((wid + x)>=1500){
            wid = 1500 - x;
        }
        if((hight + y)>=1000){
        ...
        return new Circle(x, y, color,wid, hight);
    }
}
